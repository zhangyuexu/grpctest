...
do
#备份一份原始的proto文件
cp ${INIT_PATH}${file_name[i]} ${PROTO_PATH}
...
for (( i=0;i<${#file_name[@]};i++ ))
do
  echo "filename$i="${file_name[i]}
  #mkdir -p /Users/xueqiu/Downloads/mygo/src/xq/cmd/${file_name[i]%.*}
  #$(echo $s1 | grep "${s2}")
  result=$(cat ${INIT_PATH}${file_name[i]} | grep "import")
  if [[ "$result" != ""  ]]
  then
  mkdir -p /Users/xueqiu/Downloads/mygo/src/xq/cmd/${file_name[i]%.*}
  # generate the gRPC code
  protoc -I$INIT_PATH -I. ${GOPATHLIST} --go_out=plugins=grpc:. ${INIT_PATH}${file_name[i]}
  # generate the JSON interface code
  protoc -I$INIT_PATH -I. ${GOPATHLIST} --grpc-gateway_out=logtostderr=true:. ${INIT_PATH}${file_name[i]}
  cp ${file_name[i]%.*}.* /Users/xueqiu/Downloads/mygo/src/xq/cmd/${file_name[i]%.*}/
  else
  mkdir -p /Users/xueqiu/Downloads/mygo/src/xq/cmd/pub
  protoc -I$INIT_PATH -I. ${GOPATHLIST} --go_out=plugins=grpc:. ${INIT_PATH}${file_name[i]}
  cp ${file_name[i]%.*}.* /Users/xueqiu/Downloads/mygo/src/xq/cmd/pub

  file_name3[k]=${file_name[i]}
  let k=${k}+1
  fi
